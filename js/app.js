...
const app = {
  ...
  postType: '',
  postFmts: new Set(),
  // desc espelha o texto de .type-desc de cada .type-card em index.html —
  // mantenha os dois sincronizados manualmente se um dos lados mudar.
  postTypeConfig: {
    'frase-impacto': { label:'Frase de impacto', desc:'Frase marcante ou provocativa em destaque para gerar identificação rápida e compartilhamentos.' },
    'checklist':     { label:'Checklist', desc:'Lista escaneável de itens, dicas ou erros comuns para facilitar a leitura rápida e incentivar salvamentos.' },
    'estatistica':   { label:'Estatística / Dado', desc:'Apresentação de dado de mercado ou métrica relevante em tamanho grande para ancorar um argumento ou dor.' },
    'antes-depois':  { label:'Antes e depois', desc:'Contraste visual direto entre o cenário problemático atual (Antes) e o cenário ideal pós-solução (Depois).' },
    'passo-a-passo': { label:'Passo a passo', desc:'Guia estruturado ou tutorial sequencial detalhando etapas práticas para alcançar um resultado desejado.' },
    'pergunta':      { label:'Pergunta provocativa', desc:'Questão reflexiva desenhada para tocar numa dor profunda e incentivar respostas nos comentários.' },
    'comparativo':   { label:'Comparativo X vs Y', desc:'Confronto direto de abordagens, hábitos ou ferramentas para clarear os benefícios da sua opção.' },
    'anuncio':       { label:'Anúncio / Oferta', desc:'Chamada comercial direta focando nos benefícios, preço especial e CTA (Call To Action) claro para compra.' },
    'urgencia':      { label:'Urgência / Prazo', desc:'Mensagem focada na escassez (limite de vagas, bônus ou tempo) para acelerar a tomada de ação imediata.' },
    'lancamento':    { label:'Lançamento / Novidade', desc:'Anúncio oficial de novos produtos, funcionalidades, eventos ou a abertura oficial de novas turmas/carrinho.' },
    'depoimento':    { label:'Depoimento', desc:'Citação real e prova social de um cliente atestando a transformação obtida com a marca ou produto.' },
    'citacao':       { label:'Citação de especialista', desc:'Frase de relevância ou conceito cunhado por uma autoridade do mercado para gerar prestígio e validação.' },
    'mini-artigo':   { label:'Mini artigo', desc:'Conteúdo conceitual mais denso, composto por título, introdução e corpo, focado em educar e elevar consciência.' },
  },
      ...
      return `
        <div class="app-brand-list-item${isActive ? ' active' : ''}" onclick="app.openBrand('${b.id}');document.getElementById('brandSwitcherDropdown').style.display='none';">
          <div style="display:flex; align-items:center; gap:6px; overflow:hidden; flex:1;">
            <span class="app-brand-switcher-name">${b.name}</span>
            ${isShared ? `
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#22C55E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;" title="Marca Compartilhada">
