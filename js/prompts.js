...
6. **COMPOSIÇÃO VISUAL DOS SLIDES (CRÍTICO):**
   ...
   - **Tipografia exata:** Aplique os tamanhos especificados em TAMANHO TÍTULO, TAMANHO SUBTÍTULO e TAMANHO CORPO do BASE rigorosamente no CSS, exatamente como informados — **não existe limite máximo**: se o valor especificado for grande (ex: 120px, 160px, 200px), aplique-o exatamente assim, sem reduzir por conta própria por parecer "grande demais" ou fora do convencional. Se não especificados, use escala proporcional ousada — título ≥ 2,5× o corpo, subtítulo ≈ 1.4× o corpo — evitando escalas tímidas e pouco contrastadas.
   - **NOTAS de ## 03 · TIPOGRAFIA são regra obrigatória, não sugestão:** qualquer instrução ali (ex: uso de caixa alta, restrição de mistura de fontes, tratamento específico de peso ou itálico) tem prioridade sobre convenções gerais de design e deve ser seguida à risca em todos os slides — não ignore, não suavize e não substitua por uma interpretação "mais segura".
...
6. **TIPOGRAFIA EXATA (CRÍTICO):**
   ...
   - Aplique os tamanhos especificados em TAMANHO TÍTULO, TAMANHO SUBTÍTULO e TAMANHO CORPO do BASE rigorosamente no CSS (ex: se TAMANHO TÍTULO = "32px", use exatamente \`font-size: 32px\` no headline — não ajuste para suas preferências). **Não existe limite máximo**: se o valor especificado for grande (ex: 120px, 160px, 200px), aplique-o exatamente assim, sem reduzir por conta própria por parecer "grande demais". Se tamanhos não foram especificados, use escala proporcional: título ≥ 2× o corpo, subtítulo ≈ 1.4× o corpo.
   - Aplique o PESO DOS TÍTULOS (PESO TÍTULO) especificado. Se não especificado, use 700 (Bold) como mínimo para headlines.
   - **NOTAS de ## 03 · TIPOGRAFIA são regra obrigatória, não sugestão:** qualquer instrução ali (ex: uso de caixa alta, restrição de mistura de fontes, tratamento específico de peso ou itálico) tem prioridade sobre convenções gerais de design e deve ser seguida à risca — não ignore, não suavize e não substitua por uma interpretação "mais segura".
